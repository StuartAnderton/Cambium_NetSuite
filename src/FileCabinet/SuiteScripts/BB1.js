...
define(['N/http', 'N/https', 'N/ui/serverWidget', 'N/email', 'N/runtime', 'N/task', 'N/search', 'N/redirect', 'N/record', 'N/url'],
    ...
    function (http, https, serverWidget, email, runtime, task, search, redirect, record, url) {
        ...
        function createForm(form, supplierValue, buyerValue, sortItem, itemWithNoPrefVend_chk, itemVendorNotPreferred, available) {
            var relativePath = '';
            var output = '';
            ...
            employeeSearch.run().each(function (srchResult) {
                if (srchResult.getValue({name: 'custentity_bb1_buyer', summary: 'GROUP'}) !== '')
                    buyer.addSelectOption({
                        value: srchResult.getValue({name: 'custentity_bb1_buyer', summary: 'GROUP'}),
                        text: srchResult.getText({name: 'custentity_bb1_buyer', summary: 'GROUP'})
                    });
            ...
            sortingPoItemSearch.run().each(function (srchResult) {
                soPoItem.addSelectOption({
                    value: srchResult.getValue({name: 'custrecord_bb1_oi_sorting_critera'}),
                    text: srchResult.getValue({name: 'name'})
                });
                return true;    // continue iteration
        ...
        if(supplierValue !=='')
            ...
            if (supplierValue !== null) {
                ...
                for (i = 0; i < itemResultSet.length; i++) {
                    ...
                    tempObj.item_mov = itemResultSet[i].getValue({
                        name: 'custentity_bb1_minimum_order_value',
                        join: 'vendor'
                    });
                    ...
                    tempObj.item_lastmodifieddate = itemResultSet[i].getValue({
                ...
                if (supplierList.length > 0) {
                    ...
                    lastPOSupplierSearch.run().each(function (srchResult) {
                        ...
                        lastPOID = srchResult.getValue({
                            name: 'internalid',
                            join: null,
                            summary: search.Summary.MAX
                        }) || '';
                        lastPOdate = srchResult.getValue({
                        ...
                        objSupplierlastPO.id = srchResult.getValue({
                            name: 'internalid',
                            join: 'vendor',
