...
define(['N/https', 'N/runtime', 'N/search', 'N/record'],
    ...
    function (https, runtime, search, record) {
        ...
        function afterSubmit(context) {
            ...
            var itemExternalId = item.getValue('externalid');
            ...
            var columnsToCheck = itemSearch.columns

            //log.debug('Filters', defaultFilters)
            log.debug('Columns', itemSearch.columns)

            searchResults = itemSearch.run();

            var itemData = searchResults.getRange({start: 0, end: 1});

            var values = itemData[0].getValue('values');
            log.debug('Search Results', values)
            ...
            payload.id = itemId;
            payload.name = itemExternalId;



            for (var i = 0; i < columnsToCheck.length; i++) {
                var attributeToCompare =  columnsToCheck[i].name;
                if (attributeToCompare === 'custitem_created_on') {continue}
                var oldValue = oldItem.getValue(attributeToCompare);


                var newValue = values[ attributeToCompare ];
                log.debug('New Value', newValue)
                if ((oldValue != newValue) && !(oldValue === null && newValue === '') && !(oldValue.length === 0 && newValue.length === 0)) {
                    tempObj = {};
                    tempObj.attribute = attributeToCompare;
                    tempObj.value = newValue;
                    changes.push(tempObj);
                    log.debug('Difference', [oldValue, newValue])
